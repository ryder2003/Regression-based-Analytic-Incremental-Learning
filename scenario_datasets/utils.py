...
class DatasetWrapper(TorchDataset):
    def __init__(self, data_source, input_size, transform=None, is_train=False,
        ...
        self.augmentation_time = augmentation_time if is_train else 1  # dataset扩充到相应倍数
    ...
    def __getitem__(self, idx):
        ...
        # if original_idx == 1 and self.is_train and augmentation_idx == 0:
        #     # img_pil = to_pil_image(img0)
        #     plt.imshow(img0)
        #     plt.axis('off')
        #     plt.show()

        if augmentation_idx > 0:  # not original group of images
        ...
        if self.return_img0:  # 不加augmentation
